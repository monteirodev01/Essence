...
const Promocoes: React.FC = () => {
  const options = [
    { label: "Relevância", value: "1" },
    { label: "Menor preço", value: "2" },
    { label: "Maior preço", value: "3" },
    { label: "Mais vendido", value: "4" },
    { label: "Promoção", value: "5" },
  ];
  ...
  return (
    <section className={`container ${css.gridContainer}`}>
        ...
        {products.map((product) => (
          <div className={css.produtoItem}>
            ...
            <span className={`font-1-s ${css.itemPreco}`}>
              R${product.preco}{" "}
              <span className={`font-1-xs ${css.itemDesconto}`}>
                -{product.desconto}%
              </span>
            </span>
            <button className={`font-1-xs ${css.itemBtn}`}>
              <img className={css.btnIcon} src={addToCar} alt="" /> ADD TO CART
            </button>
          </div>
        ))}
      </ul>
      ...
      <div className={`${css.loadMore}`}>
        <button className={`font-1-xs ${css.plusBtn}`}>CARREGAR MAIS</button>
      </div>
    </section>
  );
};
